...
class SaveCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        filepath = str(root_path) + "/tf2_testing/LIF_EI/sparse/set3/begin_epoch_" + str(epoch) + ".hdf5"
        #filepath = str(root_path) + "/tf2_testing/LIF/p" + str(int(p*100)) + "/begin_epoch_" + str(epoch) + ".hdf5"
        self.model.save_weights(filepath)

    def on_epoch_end(self, epoch, logs=None):
        filepath = str(root_path) + "/tf2_testing/LIF_EI/sparse/set3/end_epoch_" + str(epoch) + ".hdf5"
...
class PlotCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        plt.draw()
        plt.savefig(os.path.expanduser(os.path.join(root_path, 'tf2_testing/LIF_EI/sparse/set3/test_epoch_{}.png'.format(epoch))), dpi=300)
