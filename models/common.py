...
class SynchronyRateVoltageRegularization(tf.keras.layers.Layer):
    def __init__(
        ...
        rate_cost,
        voltage_cost,
    ):  # fano factor-like rapid synchrony measure
        super().__init__()
    ...
    def call(self, inputs, **kwargs):
        ...
        synchrony = fano_factor(
            self, seq_len, spike
        )  # for individual units across trial
        global_synchrony = tf.reduce_mean(
            synchrony
        )  # across all units and trial time
        self.add_metric(
            global_synchrony, name="synchrony", aggregation="mean"
        )

        synch_reg_loss = (
            tf.reduce_sum(tf.square(synchrony - self._target_synch))
            * self._synch_cost
        )
        ...
        rate_reg_loss = (
            tf.reduce_sum(tf.square(unitwise_rates - self._target_rate))
            * self._rate_cost
        )
        ...
        voltage_loss = (
            tf.reduce_sum(
                tf.square(tf.math.abs(voltage) - tf.math.abs(self._cell.EL))
            )
            * self._voltage_cost
        )
...
class GlobalSpikeRegularization(tf.keras.layers.Layer):
    ...
    def __init__(
        self, cell, target_rate, rate_cost
    ):  # rate in spikes/ms for ease
        """TODO: docs"""
        super().__init__()
        self._rate_cost = rate_cost
        self._target_rate = target_rate
    ...
    def call(self, inputs, **kwargs):
        ...
        global_rate = tf.reduce_mean(
            rate
        )  # I think this is for the whole batch of trials?
        self.add_metric(global_rate, name="rate", aggregation="mean")

        reg_loss = (
            tf.reduce_sum(tf.square(rate - self._target_rate))
            * self._rate_cost
        )
        self.add_loss(reg_loss)
        self.add_metric(reg_loss, name="rate_loss", aggregation="mean")
...
class SpikeVoltageRegularization(tf.keras.layers.Layer):
    ...
    def __init__(
        self, cell, target_rate, rate_cost, voltage_cost
    ):  # rate in spikes/ms for ease
        """TODO: docs"""
        self._rate_cost = rate_cost
        self._voltage_cost = voltage_cost
...
def fano_factor(self, seq_len, spike):
    ...
    for i in range(0, n_bins):
        ...
        spike_slice = tf.gather(
            spike, range(i * len_bins, (i + 1) * len_bins), axis=1
        )
        ...
        fano_bin = tf.math.divide_no_nan(
            tfp.stats.variance(spikes_per_neuron, sample_axis=None),
            tf.reduce_mean(spikes_per_neuron),
        )
...
def exp_convolve(tensor, decay=0.8, reverse=False, initializer=None, axis=0):
    ...
    filtered = tf.scan(
        scan_fun, tensor, reverse=reverse, initializer=initializer
    )
