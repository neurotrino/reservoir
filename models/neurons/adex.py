...
class _AdExCore(Neuron):
    ...
    def build(self, input_shape, connmat_generator, initializers):
        ...
        if self.rewiring:
            ...
            self.rec_sign = tf.convert_to_tensor(
                wmat, dtype=tf.float32
            )  # +1 for excitatory and -1 for inhibitory
        else:
            self.rec_sign = tf.sign(
    ...
    def call(self, inputs, state):
        ...
        new_v = (
            old_v
            - (self.dt_gL__C * (old_v - self.EL))
            + (
                self.dt_gL__C
                * self.deltaT
                * tf.clip_by_value(
                    tf.exp((old_v - self.thr) / self.deltaT),
                    -1e6,
                    30 / self.dt_gL__C,
                )
            )
            + ((i_t - old_w) * self._dt / self.C)
        )
        new_v = tf.where(
            old_z > 0.5, tf.ones_like(new_v) * self.V_reset, new_v
        )

        # Update adaptation term
        new_w = (
            old_w
            - ((self._dt / self.tauw) * old_w)
            + (self.dt_a__tauw * (old_v - self.EL))
        )
        new_w += self.b * old_z
        ...
        new_r = tf.clip_by_value(
            old_r - 1 + tf.cast(new_z * self.n_refrac, tf.int32),
            0,
            self.n_refrac,
        )
    ...
    ):  # is this not in Neuron? does it differ from LIF?
        sz = (batch_size, self.units)

        # Voltage (all at EL)
        v0 = (
            self.v0_sdist.sample(sz) + self.EL
        )  # Do we want to start with random V?
        # Refractory (all 0)
