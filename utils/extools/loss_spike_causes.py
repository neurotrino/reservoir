...
experiment = "fwd-pipeline-inputspikeregen-newl23-owerlr-runlonger-norewire"
savepath = "/data/results/fwd/loss_causes_over_time_0.001_norewire.png"
...
def scatter_reasons():
    ...
    for filename in data_files:
        ...
        for i in range(0, len(w_main)):
            ...
            e = w[w > 0]
            i = w[w < 0]
            eiratio_main = np.append(
                eiratio_main, np.abs(np.sum(e) / np.sum(i))
            )
        ...
        for i in range(0, len(w_out)):
            ...
            e = w[w > 0]
            i = w[w < 0]
            eiratio_out = np.append(
                eiratio_out, np.abs(np.sum(e) / np.sum(i))
            )
    ...
    fig.suptitle("main lr 0.001, output lr 0.00001, no rewiring")
