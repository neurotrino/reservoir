...
def test_local():
    ...
    end_fname = (
        "ccd_data/alif/npz-data/"
        + epoch_groups[len(epoch_groups) - 1]
        + ".npz"
    )
...
def plot_density(experiment):  # July 20, 2021: for sake of testing rewiring
    ...
    plt.savefig(
        "../../../experiments/"
        + experiment
        + "/analysis/alif_to_epoch_70.png",
        dpi=300,
    )


def plot_data_over_epochs(
    experiment,
):  # March 23, 2021: creating plots for Graz meeting
    sims = 10
    mu = -0.64
    sigma = 0.51
    dist = [mu, sigma]
    ...
    plt.draw()
    plt.savefig(
        "../experiments/" + experiment + "/analysis/output_figure_to_160.png",
        dpi=300,
    )
...
def w_motif_propensity(rec_w, sims, dist):
    ...
    for i in range(sims):
        ...
        for j in indices[0]:
            ...
            sample_mat[indices[0][j], indices[1][j]] = np.random.lognormal(
                dist[0], dist[1]
            )
...
def motifs_cc(rec_w):
    W = np.transpose(
        rec_w
    )  # Wij convention in Fagiolo is the transpose of ours
    temp = np.cbrt(W)
    ...
    fanin_CC = np.divide(
        np.diag(np.matmul(np.transpose(temp), np.square(temp))),
        np.multiply(d_in, d_in - 1),
    )
    fanout_CC = np.divide(
        np.diag(np.matmul(np.square(temp), np.transpose(temp))),
        np.multiply(d_out, d_out - 1),
    )
    middleman_CC = np.divide(
        np.diag(np.matmul(np.matmul(temp, np.transpose(temp)), temp)), denom
    )
...
def process_hdf5(epoch):
    hf = h5py.File(
        "minimal_sparsity_data/v2/logs/set4/end_epoch_"
        + str(epoch)
        + ".hdf5",
        "r",
    )
