...
def compare_change_over_training():
    ...
    for xdir in experiments:
        ...
        plt.suptitle("Change in weights from naive to task-trained")

        # Draw and save
        plt.draw()
        plt.subplots_adjust(wspace=0.4, hspace=0.7)
        save_fname = savepath+task_exp_path+'/'+exp_path+'_change_in_w_task.png'
...
def plot_subpopulation_sum_weights():
    ...
    for xdir in experiments:
        ...
        # Label and title
        ax[0,0].set_title('epoch 0')
        ax[0,0].set_xlabel('sum weights')
        #ax[0,0].legend()
        ax[0,1].set_title('epoch 50')
        ax[0,1].set_xlabel('sum weights')
        #ax[0,1].legend()
        ax[1,0].set_title('epoch 250')
        ax[1,0].set_xlabel('sum weights')
        #ax[1,0].legend()
        ax[1,1].set_title('epoch 1000')
        ...
        plt.draw()
        plt.subplots_adjust(wspace=0.4, hspace=0.7)
        save_fname = savepath+exp_path+'/'+exp_path+'_rec_sum_subpop_weights_all.png'
        plt.savefig(save_fname,dpi=300)
...
def plot_in_v_out_strength():
    ...
    for xdir in experiments:
        ...
        ax[0,0].scatter(np.sum(naive_in,0),naive_out.flatten(),s=2)
        ax[0,1].scatter(np.sum(early_in,0),early_out.flatten(),s=2)
        ax[1,0].scatter(np.sum(late_in,0),late_out.flatten(),s=2)
        ax[1,1].scatter(np.sum(trained_in,0),trained_out.flatten(),s=2)
        ...
        plt.suptitle("Evolution of input vs output weights per neuron, task only")

        # Draw and save
        plt.draw()
        plt.subplots_adjust(wspace=0.4, hspace=0.7)
        save_fname = savepath+exp_path+'/'+exp_path+'_input_v_output_all_quad.png'
        plt.savefig(save_fname,dpi=300)
