...
def plot_input_populations(): # to those units in the RSNN that send to output vs ones that do not
    ...
    for xdir in experiments:
        ...
        ax[0,0].hist((naive_in[:,naive_not_id]).tolist(),bins=50,histtype='step',label='no output conn')
        ax[0,0].hist((naive_in[:,naive_out_id]).tolist(),bins=50,histtype='step',label='output conn')

        ax[0,1].hist((early_in[:,early_not_id]).tolist(),bins=50,histtype='step',label='no output conn')
        ax[0,1].hist((early_in[:,early_out_id]).tolist(),bins=50,histtype='step',label='output conn')

        ax[1,0].hist((late_in[:,late_not_id]).tolist(),bins=50,histtype='step',label='no output conn')
        ax[1,0].hist((late_in[:,late_out_id]).tolist(),bins=50,histtype='step',label='output conn')

        ax[1,1].hist((trained_in[:,trained_not_id]).tolist(),bins=50,histtype='step',label='no output conn')
        ax[1,1].hist((trained_in[:,trained_out_id]).tolist(),bins=50,histtype='step',label='output conn')
