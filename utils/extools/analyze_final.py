...
def plot_input_grouped_rec_weights(exp_dirs=spec_nointoout_dirs,exp_season='spring'):
    ...
    for xdir in exp_data_dirs:
        ...
        # plot
        fig, ax = plt.subplots(nrows=1, ncols=2)
        ax[0].hist(np.flatten(w[coh0_e,:][:,coh0_e]),bins=22,density=True)
        ax[0].hist(np.flatten(w[coh0_e,:][:,coh0_i]),bins=22,density=True)
        ax[0].hist(np.flatten(w[coh0_i,:][:,coh0_e]),bins=22,density=True)
        ax[0].hist(np.flatten(w[coh0_i,:][:,coh0_i]),bins=22,density=True)
        ax[0].set_title('0-driven units',fontname='Ubuntu')

        ax[1].hist(np.flatten(w[coh1_e,:][:,coh1_e]),bins=22,density=True)
        ax[1].hist(np.flatten(w[coh1_e,:][:,coh1_i]),bins=22,density=True)
        ax[1].hist(np.flatten(w[coh1_i,:][:,coh1_e]),bins=22,density=True)
        ax[1].hist(np.flatten(w[coh1_i,:][:,coh1_i]),bins=22,density=True)
