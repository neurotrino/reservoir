...
def plot_all_weight_dists(exp_dirs=spec_nointoout_dirs,exp_season='spring'): # just for dual-training for now
    ...
    sns.kdeplot(in_e_naive[in_e_naive!=0],color='slateblue',label='to e',ax=ax[0])
    sns.kdeplot(in_i_naive[in_i_naive!=0],color='orangered',label='to i',ax=ax[0])
    ax[0].set_title('naive input',fontname='Ubuntu')
    sns.kdeplot(in_e_trained[in_e_trained>0],color='slateblue',label='to e',ax=ax[1])
    ...
    plt.draw()
    save_fname = savepath+'/set_plots/'+exp_season+'_all_input_weights_test.png'
    ...
    sns.kdeplot(out_e_naive[out_e_naive!=0],color='slateblue',label='from e',ax=ax[0])
    sns.kdeplot(out_i_naive[out_i_naive!=0],color='orangered',label='from i',ax=ax[0])
    ax[0].set_title('naive output',fontname='Ubuntu')
    sns.kdeplot(out_e_trained[out_e_trained!=0],color='slateblue',label='from e',ax=ax[1])
    ...
    for i in range(0,len(ax)):
        ...
        ax[i].set_ylabel('density',fontname='Ubuntu')
    plt.draw()
    save_fname = savepath+'/set_plots/'+exp_season+'_all_output_weights_test.png'
    plt.savefig(save_fname,dpi=300)
    # Teardown
    plt.clf()
    plt.close()
    ...
    fig, ax = plt.subplots(nrows=2,ncols=1)
    rec_naive_ee = rec_naive[:,0:e_end,0:e_end].flatten()
    rec_naive_ei = rec_naive[:,0:e_end,e_end:0].flatten()
    rec_naive_ie = rec_naive[:,e_end:0,0:e_end].flatten()
    rec_naive_ii = rec_naive[:,e_end:0,e_end:0].flatten()
    rec_trained_ee = rec_trained[:,0:e_end,0:e_end].flatten()
    rec_trained_ei = rec_trained[:,0:e_end,e_end:0].flatten()
    rec_trained_ie = rec_trained[:,e_end:0,0:e_end].flatten()
    rec_trained_ii = rec_trained[:,e_end:0,e_end:0].flatten()

    sns.kdeplot(rec_naive_ee[rec_naive_ee!=0],color='slateblue',label='ee',ax=ax[0])
    sns.kdeplot(rec_naive_ei[rec_naive_ei!=0],color='mediumseagreen',label='ei',ax=ax[0])
    ...
    sns.kdeplot(rec_trained_ee[rec_trained_ee!=0],color='slateblue',label='ee',ax=ax[1])
    sns.kdeplot(rec_trained_ei[rec_trained_ei!=0],color='mediumseagreen',label='ei',ax=ax[1])
