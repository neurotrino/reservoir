...
def plot_avalanche_dist(threshold_range=True,bin_range=False,subsample=False):
    ...
    naive_spikes = naive_data['spikes'][299]
    ...
    if threshold_range:
        ...
        plt.suptitle('E avalanche size dist; dual trained; two pops')
        # Draw and save
        plt.draw()
        plt.subplots_adjust(wspace=0.4, hspace=0.96)
        save_fname = savepath+'/criticality/avalanches_e_dualtrained_epoch50_thresholds_twopops_22.44.51.png'
        plt.savefig(save_fname,dpi=300)
        ...
        for bin_idx in range(len(bin_sizes)):
            ...
            for i in range(0,np.shape(naive_spikes)[0]):
                ...
                Z = fastbin(np.transpose(prebin_Z), bin_sizes[bin_idx], np.shape(prebin_Z)[1])
        ...
        plt.suptitle('E avalanche size dist; task trained; 40%ile threshold')
        # Draw and save
        plt.draw()
        plt.subplots_adjust(wspace=0.4, hspace=0.96)
        save_fname = savepath+'/criticality/avalanches_e_tasktrained_thresholds_coarse_11.54.39.png'
        plt.savefig(save_fname,dpi=300)
...
def fastbin(z, bin_sz, num_units):
...
def get_binned_spikes_trialends(e_only, true_y, spikes, bin):
    ...
    for trial in range(true_y.shape[0]):
        ...
        if coh_0_idx.size > 0:
            ...
            trial_binned_z = fastbin(z_coh0, bin, n_units)
            ...
            trial_binned_z = fastbin(z_coh1, bin, n_units)
