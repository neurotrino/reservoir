...
def single_epoch_lr(experiment, epoch_of_interest, batch_of_interest):
    ...
    datapath = os.path.join(data_dir, experiment, 'npz-data', epoch_of_interest)
    ...
    in_lr = np.divide(np.abs(new_in_w - old_in_w), np.abs(in_grad), out=np.zeros_like(new_in_w - old_in_w), where=in_grad!=0)
    main_lr = np.divide(np.abs(new_main_w - old_main_w), np.abs(main_grad), out=np.zeros_like(new_main_w - old_main_w), where=main_grad!=0)
    out_lr = np.divide(np.abs(new_out_w - old_out_w), np.abs(out_grad), out=np.zeros_like(new_out_w - old_out_w), where=out_grad!=0)
