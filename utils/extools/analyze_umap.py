...
def map_no_labels():
    ...
    for xdir in data_dirs:
        ...
        # aggregate spike data with trial as the first dim
        all_data_arr.append(all_data)
        all_labels = np.concatenate((naive_y, trained_y), axis=0).flatten()
    ...
    exp_string = xdir[-9:-1]  # NOTE: not used
    save_fname = savepath+set_save_name+'/umap.png'
